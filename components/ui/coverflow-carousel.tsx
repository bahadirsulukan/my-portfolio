...
export interface CoverflowSlide {
  ...
  /** Local addition: renders a call-to-action under the caption. */
  href?: string;
  /** Local addition: label for that link. */
  hrefLabel?: string;
}
...
export interface CoverflowCarouselProps {
  ...
  cardWidth?: string;
  /** Local addition: card width ÷ height. 1 keeps the original square; a
      wider ratio suits screenshots, which are landscape to begin with. */
  cardAspect?: number;
  /** Space between cards, as a fraction of card width. */
  gap?: number;
  loop?: boolean;
  /** Local addition: drive the carousel from outside — e.g. from scroll
      position. Changing it settles to that slide; manual input still works
      in between. Leave undefined to keep the carousel self-governed. */
  activeIndex?: number;
  showCaption?: boolean;
...
export function CoverflowCarousel({
  ...
  // Local addition: follow an externally driven index. Guarded so it only
  // fires when the requested slide actually differs — otherwise every parent
  // re-render would restart the settle and fight the user's own dragging.
  React.useEffect(() => {
    if (activeIndex == null) return;
    if (indexAt(targetRef.current) === activeIndex) return;
    goTo(activeIndex);
  }, [activeIndex, goTo, indexAt]);
      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-5 w-full max-w-[230px] text-[12px]">
              {active.meta.map((row) => (
          ...
          {/* Local addition: without this the carousel would be the only part
              of the portfolio you cannot click through to the real project. */}
          {active.href && (
            <a
              href={active.href}
              target="_blank"
              rel="noreferrer"
              className="mt-5 rounded-md border border-ring/50 px-4 py-2 text-[12px] font-semibold tracking-wide text-foreground transition hover:bg-foreground/5"
            >
              {active.hrefLabel ?? "View project"} →
            </a>
          )}
        </div>
      )}
